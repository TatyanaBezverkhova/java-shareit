...
class ItemRepositoryTest {
    ...
    void findItemByRequestJpaTest() {
        ...
        assertThat(itemPersist).isEqualTo(items.get(0));
        assertThat(itemPersist.getId()).isEqualTo(items.get(0).getId());
    }
    ...
    @Test
    void findByOwnerWithPageableJpaTest() {
        addItem();
        Item itemPersist = em.persist(item);
    ...
    @Test
    void findItemsByNameOrDescriptionWithPageableJpaTest() {
        addItem();
        Item itemPersist = em.persist(item);
        Pageable pageable = PageRequest.of(0, 1);
        Page<Item> itemsPage = itemRepository.findItemsByNameOrDescription("F", pageable);
        List<Item> items = itemsPage.getContent();
        assertThat(itemPersist).isEqualTo(items.get(0));
        assertThat(itemPersist.getId()).isEqualTo(items.get(0).getId());
    }
